    ...
    for (i=k=0;i<num_used_paths;i++)
    {
        ...
        for (j=0 ; j < uptr->numfixedgenes ; j++)
        {
            ...
            *(augreverse+k) = uptr; // since unlabelled, essential that this array of pointers has the same order as auguniverse
            k++;
            if (k > aug_gene_ct) // was just > -- ( rpf xxx )
            {
...
// fprintf(stderr,"rpf in GPCC(), real_universe_cnt=%u 6\n",real_universe_cnt); fflush(NULL);
    ...
    for (ui=0 ; ui<aug_gene_ct ; ui++)
    {
         ...
         if(this_gene == 7040) ++utilctr;   // rpf - what is this ?
    }
...
int l2pfunc(struct used_path_type *usedpaths,unsigned int num_used_paths,unsigned int real_universe_cnt,unsigned int *real_universe, int permute_option, int *user_incnt_ptr, int oneside)
{
    char s[512];
    unsigned int prev;
    unsigned int ui;
...
#if 0
fprintf(stderr,"in lp2func(), before permute_test2\n"); fflush(NULL);
        permute_test2(usedpaths,num_used_paths,real_universe,real_universe_cnt,user_incnt);
fprintf(stderr,"in lp2func(), after permute_test2, before bh4()\n"); fflush(NULL);
